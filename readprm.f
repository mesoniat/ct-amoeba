...
c
c  DCT charge transfer parameters
c
         else if (keyword(1:4) .eq. 'DCT ') then
            string = record(next:120)
            read (string,*,err=65,end=65)  rct1,rct2,
     & zdqt(1,1),zdqt(2,1),zdqt(3,1),zdqt(1,3),zdqt(2,3),
     & muct, etact
c           write(*,*) "DCT param.s read in."
   65       continue
c zdqt is charge transfer matrix
            zdqt(1,2) = zdqt(1,1)
            zdqt(2,2) = zdqt(3,1)
            zdqt(3,2) = zdqt(2,1)
            zdqt(3,3) = zdqt(2,3)
