...
c     #################################################################
c     ##                                                             ##
c     ##  program dynamic_THz  --  run MD and calculated DCF         ##
c     ##                                                             ##
c     #################################################################
c
c
c     "dynamic_THz" is a modified version of dynamic which calculates
c     the dipole autocorrelation function, which is used in the 
c     calculation of the THz vibrational spectrum
c
c
      program dynamic_THz
      use sizes
      ...
      integer istep0,ngrp2
      integer ISTOREM,DTCALCM,ISKIPM,ISTOREV,DTCALCV,ISKIPV,ISTORED
      integer DTCALCD,ISKIPD,SAVE_DATA_FREQ,SAVE_REST_FREQ
c     integer ISKIPM,ISKIPV,ISKIPD
...
c
c     adding THz stuff here
c
c modify group variables for AMOEBA
      ngrp2 = n / 3

c
c read THz inputs (from cmd line)
c
      call nextarg (string,exist)
      ...
      call nextarg (string,exist)
      if (exist)  read (string,*)  SAVE_DATA_FREQ 
c Freq to write the dcf and other production arrays to file
      call nextarg (string,exist)
      if (exist)  read (string,*)  SAVE_REST_FREQ  
c Freq to write outcoordinates and velocities for restart files.
...
c     read file names for writing output
      ...
      call nextarg (string,exist)
      if (exist)  read (string,*)  FILE5 
c name of dipole correlation file
      call nextarg (string,exist)
...
c
c     initialize dipole and related calc.s
c
...
c
c     calculation of VACF
c

c
c     calculation of DCF
c
